...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (viewModeBtn && viewModeDropdown) {
    viewModeBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      viewModeDropdown.style.display = viewModeDropdown.style.display === 'block' ? 'none' : 'block';
    });
    ...
    document.addEventListener('click', function(e) {
      if (viewModeDropdown.style.display === 'block') {
        viewModeDropdown.style.display = 'none';
      }
    });
  }

  // Fix: Resume AudioContext and play ambient sound on first user interaction
...
function generateAIResponse(message) {
  const commandResult = handleAICommand(message);
...
function toggleMute() {
  ...
  const isMuted = body.classList.contains('muted');
  if (isMuted) {
    muteBtn.classList.add('muted');
    muteIcon.classList.add('hidden');
    unmuteIcon.classList.remove('hidden');
    // Mute all audio
    document.querySelectorAll('audio').forEach(a => { a.muted = true; a.pause && a.pause(); });
...
function handleAICommand(message) {
  ...
  // Theme/mood change
  if (lower.includes('theme') || lower.includes('mood')) {
    if (lower.includes('hacker')) { window.auraMoodEngine && window.auraMoodEngine.setMood('hacker'); return 'Theme changed to Hacker!'; }
    if (lower.includes('dreamy')) { window.auraMoodEngine && window.auraMoodEngine.setMood('dreamy'); return 'Theme changed to Dreamy!'; }
    if (lower.includes('calm')) { window.auraMoodEngine && window.auraMoodEngine.setMood('calm'); return 'Theme changed to Calm!'; }
    if (lower.includes('energetic')) { window.auraMoodEngine && window.auraMoodEngine.setMood('energetic'); return 'Theme changed to Energetic!'; }
    if (lower.includes('cyber pink')) { window.auraMoodEngine && window.auraMoodEngine.setMood('cyberpink'); return 'Theme changed to Cyber Pink!'; }
    if (lower.includes('electric blue')) { window.auraMoodEngine && window.auraMoodEngine.setMood('electricblue'); return 'Theme changed to Electric Blue!'; }
    if (lower.includes('auto')) { window.auraMoodEngine && window.auraMoodEngine.setMood('auto'); return 'Theme set to Auto!'; }
    return 'Please specify a valid theme.';
  }
  // View mode
  if (lower.includes('phone view')) { document.body.classList.add('phone-view'); return 'Switched to Phone View!'; }
  if (lower.includes('pc view')) { document.body.classList.remove('phone-view'); return 'Switched to PC View!'; }
  // Mute/unmute
  if (lower.includes('mute')) { if (!document.body.classList.contains('muted')) toggleMute(); return 'Sound muted!'; }
  if (lower.includes('unmute')) { if (document.body.classList.contains('muted')) toggleMute(); return 'Sound unmuted!'; }
  // Navigation
  if (lower.includes('project')) { document.getElementById('projects')?.scrollIntoView({behavior:'smooth'}); return 'Navigating to Projects!'; }
  if (lower.includes('contact')) { document.getElementById('contact')?.scrollIntoView({behavior:'smooth'}); return 'Navigating to Contact!'; }
  if (lower.includes('timeline')) { document.getElementById('timeline')?.scrollIntoView({behavior:'smooth'}); return 'Navigating to Timeline!'; }
  // Jokes
  ...
  if (lower.includes('help') || lower.includes('what can you do')) {
    return 'I can change the website theme, switch between PC/Phone view, mute/unmute sound, navigate to sections, tell jokes, and more! Try commands like: change theme to hacker, switch to phone view, mute sound, go to projects, tell me a joke.';
  }
  return null;
}
